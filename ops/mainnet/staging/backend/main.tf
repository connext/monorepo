terraform {
  backend "s3" {
    bucket = "nxtp-terraform-mainnet-staging-backend"
    key    = "state"
    region = "us-east-1"
  }
}
...
module "cartographer_db" {
  domain                = "cartographer"
  source                = "../../../modules/db"
  identifier            = "rds-postgres-cartographer-${var.stage}-${var.environment}"
...
module "cartographer_db_replica" {
  ...
  replica_identifier  = "rds-postgres-cartographer-replica-${var.stage}-${var.environment}"
