...
describe("Helpers:Shared", () => {
  ...
  describe("#getDestinationLocalAsset", () => {
    it("should return the local asset for the destination chain", async () => {
      let mockContext = stubContext();
      const mockLocalAsset = "0x456";
      mockContext.adapters.subgraph.getAssetByLocal.returns({ canonicalId: "0x123" });
      mockContext.adapters.subgraph.getAssetByCanonicalId.returns({ local: mockLocalAsset });
