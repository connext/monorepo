...
export const makeRoutersPoller = async (_configOverride?: CartographerConfig) => {
  ...
  const methodContext = createMethodContext(makeRoutersPoller.name);
  ...
  await bindRouters();
  await closeDatabase();
  if (context.config.healthUrls.routers) {
    await sendHeartbeat(context.config.healthUrls.routers, context.logger);
  }
};
