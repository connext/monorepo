...
import { CrossChainMessageProof } from "@mantleio/sdk";
import { BigNumber, providers } from "ethers";

import { MantleCrossChainMessenger } from "../../../mockable";
 ...
 * - Metis
 */
export const getProcessFromMantleRootArgs = async ({
  spokeChainId,
  hubChainId,
  spokeProvider,
  hubProvider,
  ...
  const { requestContext, methodContext } = createLoggingContext(getProcessFromMantleRootArgs.name, _requestContext);
  ...
  logger.info("Got message from mantle", requestContext, methodContext, { message });

  // get the inclusion proof
  const proof = await messenger.getMessageProof(sendHash);
  logger.info("Got proof from mantle", requestContext, methodContext, { proof });
