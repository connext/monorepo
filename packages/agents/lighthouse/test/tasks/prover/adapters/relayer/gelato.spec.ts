...
import { send } from "../../../../../src/tasks/prover/adapters/relayer/gelato";
import {
  gelatoSendStub,
...
describe("Adapters: Gelato", () => {
  describe("#send", () => {
    ...
    it("should send the bid to the relayer", async () => {
      const taskId = await send(mock.chain.A, proverCtxMock.config.chains[mock.domain.A].deployments.connext, "0xbeed");
