...
import * as UtilsFns from "@connext/nxtp-utils";
import { getRandomAddress, getRandomBytes32, expect, Logger, RequestContext, NxtpError } from "@connext/nxtp-utils";

import { Gas, SyncProvider, OnchainTransaction } from "../src/types";
...
describe("ChainRpcProvider", () => {
  ...
  describe("#sendTransaction", () => {
    ...
    it.skip("should return error if sending has been aborted", async () => {
      ...
      await expect((chainProvider as any).sendTransaction(transaction)).to.be.rejectedWith(DispatchAborted);
    });
  ...
  describe("#confirmTransaction", () => {
    ...
    it("should wait until receipt returns and confirmations are insufficient", async () => {
      ...
      expect(waitStub.getCall(0).args[0]).to.be.deep.eq(currentBlockNumber + testDesiredConfirmations);
      expect(waitStub.getCall(1).args[0]).to.be.deep.eq(currentBlockNumber + testDesiredConfirmations - insufficientConfirmations);
    ...
    it("should throw reverting transactions if no receipts are successful", async () => {
      ...
      await expect(chainProvider.confirmTransaction(transaction, testDesiredConfirmations)).to.be.rejectedWith(TransactionReverted);
    ...
    it("should not throw reverted transactions if one receipt is successful", async () => {
      ...
      getTransactionReceiptStub.callsFake((hash) => {
        ...
        return hash === revertedHash ? {
          ...
          confirmations: testDesiredConfirmations,
          status: 0,
        } : hash === successfulHash ? expectedReceipt : null;
      });
      const result = await chainProvider.confirmTransaction(transaction, testDesiredConfirmations);
      expect(makeChaiReadable(result)).to.be.deep.eq(makeChaiReadable(expectedReceipt));
    });
  ...
  describe("#estimateGas", () => {
    ...
    it("should handle invalid value for gas estimate", async () => {
      coreSyncProvider.send.resolves("thisisnotanumber");
      await expect(chainProvider.estimateGas(testTx)).to.be.rejectedWith(
        TransactionServiceFailure.reasons.GasEstimateInvalid,
      );
    });
  ...
  describe("#getBlockNumber", () => {
    ...
    it("uses cached block number if available", async () => {
      const blockNumber = 123;
      (chainProvider as any).synchronizedMode = true;
      (chainProvider as any).cache.update(blockNumber);

      const result = await chainProvider.getBlockNumber();

      expect(result).to.be.eq(blockNumber);
      expect(coreSyncProvider.getBlockNumber.callCount).to.equal(0);
    });
  });
  ...
  describe("#syncProviders", () => {
    ...
    it("happy", async () => {
      ...
      // Should have set a block listener on the (most) in-sync provider.
      expect(coreSyncProvider.addBlockListener.callCount).to.eq(1);
      expect(outOfSyncProvider.addBlockListener.callCount).to.eq(0);
    });

    it("doesn't set a block listener if not in synchronized mode", async () => {
      (chainProvider as any).synchronizedMode = false;
      (chainProvider as any).providers = [coreSyncProvider, outOfSyncProvider];
      await (chainProvider as any).syncProviders();
      // Should not have set any block listeners.
      expect(coreSyncProvider.addBlockListener.callCount).to.eq(0);
      expect(outOfSyncProvider.addBlockListener.callCount).to.eq(0);
    });
  });

  describe.skip("#wait", () => {
    // TODO: fix; find a way to stub delay util fn.
    // Alternative: just make optional param to wait() to determine delay lengths?
    it("waits until the target block", async () => {
      (chainProvider as any).synchronizedMode = true;

      // Simulate the block number increasing each time delay is called.
      const initialBlockNumber = 100;
      const targetBlockNumber = 123;
      let currentBlockNumber = initialBlockNumber;
      console.log("1", UtilsFns.delay);
      const delayStub = Sinon.stub(UtilsFns, "delay").callsFake(async (_: number) => {
        currentBlockNumber++;
        (chainProvider as any).cache.update(currentBlockNumber);
      });
      (chainProvider as any).cache.update(initialBlockNumber);
      Object.defineProperty(global, "delay", { value: delayStub, configurable: true, writable: true });
      console.log("2", UtilsFns.delay);

      await (chainProvider as any).waitForBlock(targetBlockNumber);
      expect(delayStub.callCount).to.be.eq(targetBlockNumber - initialBlockNumber);
    });

    it.skip("waits a preset amount of delay", async () => {});
  });
