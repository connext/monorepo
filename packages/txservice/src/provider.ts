...
export class ChainRpcProvider {
  ...
  private synchronizedMode: boolean;
  ...
  constructor(
    ...
    this.cache = new ProviderCache<{ gasPrice: BigNumber; transactionCount: number }>(this.logger, {
      ...
      transactionCount: this.synchronizedMode
        ? {
            btl: 0,
          }
        : {
  ...
  protected async sendTransaction(transaction: OnchainTransaction): Promise<providers.TransactionResponse> {
    ...
    // making fallback provider obsolete.
  ...
  public async confirmTransaction(
    ...
    // If we're using an active block listener, we can optimize by waiting for the target # of blocks.
    if (this.synchronizedMode && transaction.minedBlockNumber !== -1 && confirmations > 1) {
    ...
    while (!timedOut) {
      ...
      if (!timedOut) {
        ...
        await this.wait(this.synchronizedMode ? this.cache.blockNumber + remainingConfirmations : undefined);
      }
    }
    ...
    return this.execute(false, async (provider: SyncProvider) => {
      ...
      try {
        ...
        throw new TransactionServiceFailure(TransactionServiceFailure.reasons.GasEstimateInvalid, {
          invalidEstimate: result ? result.toString() : "null",
          gasLimitInflation,
          error: error.message,
        });
      }
  ...
  public async getBlockNumber(): Promise<number> {
    // Check to make sure we have an active block listener before using the cached block number value.
    if (this.synchronizedMode && this.cache.blockNumber) {
  ...
  protected async syncProviders(): Promise<void> {
    ...
    if (this.synchronizedMode) {
      // If we're running in synchronized mode, activate the block listener used for keeping the cache updated.
      this.leadProvider.addBlockListener((blockNumber: number) => this.cache.update(blockNumber));
    }
  ...
  /**
   * Helper method to stall, possibly until we've reached a certain block number. Only works
   * with block number if we're running in synchronized mode.
   *
   * @param blockNumber - the block number to wait for.
   */
  private async wait(blockNumber?: number) {
    // TODO: This could be optimized per chain, etc.
    const pollPeriod = 2_000;
    if (this.synchronizedMode && blockNumber) {
      // TODO: This could use an event listener to avoid redundant threads of waiting loops.
      while (this.cache.blockNumber < blockNumber) {
