...
const getPriceOracleInterface = () => new Interface(PriceOracleArtifact.abi) as TConnextPriceOracle["interface"];

export class PriceOracle {
  constructor(
    private readonly logger: Logger,
    private readonly readTx: (tx: ReadTransaction) => Promise<string>,
    private readonly getGasPrice: (chainId: number, requestContext: RequestContext) => Promise<BigNumber>,
  ) {}
   ...
   * @param oracleContractAddress - The address of the price oracle contract.
   */
  async getTokenPrice(chainId: number, assetId: string, oracleContractAddress: string): Promise<BigNumber> {
  ...
  async calculateGasFeeInReceivingTokenForPrepare(
    ...
    if (chaindIdsForGasFee.includes(sendingChainId)) {
      ...
      const [ethPriceInSendingChain, receivingTokenPrice, gasPriceInSendingChain] = await Promise.all([
        this.getTokenPrice(tokenPricingSendingChain, constants.AddressZero, sendingOracleContractAddress),
        this.getTokenPrice(tokenPricingSendingChain, sendingAssetId, receivingOracleContractAddress),
    ...
    if (chaindIdsForGasFee.includes(receivingChainId)) {
      ...
      const [ethPriceInReceivingChain, receivingTokenPrice, gasPriceInReceivingChain] = await Promise.all([
        this.getTokenPrice(tokenPricingReceivingChain, constants.AddressZero, receivingOracleContractAddress),
        this.getTokenPrice(tokenPricingReceivingChain, receivingAssetId, receivingOracleContractAddress),
