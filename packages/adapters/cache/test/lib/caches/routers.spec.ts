...
describe("AuctionCache", () => {
  ...
  describe("#getApproval", () => {
    it("happy: should retrieve existing approval status", async () => {
      const domain = mock.domain.A;
      const router = mock.address.router;
      const approval = true;

      await mockRedisHelpers.setApproval(domain, router, approval);
      const res = await cache.getApproval(domain, router);

      expect(res).to.be.eq(approval);
    });

    it("should retrieve false approval status as well", async () => {
      const domain = mock.domain.A;
      const router = mock.address.router;
    ...
    it("sad: should return undefined if approval status doesn't exist", async () => {
      const domain = mock.domain.B;
      const router = mock.address.router;
    ...
    it("should return undefined if approval status is expired", async () => {
      const domain = mock.domain.A;
      const router = mock.address.router;
      const approval = true;

      await mockRedisHelpers.setApproval(
        domain,
        router,
        approval,
        // Subtract another 10 secs to be safe.
  ...
  describe("#setApproval", () => {
    it("happy: should set approval status", async () => {
      const domain = mock.domain.A;
      const router = mock.address.router;
      const approval = true;

      await cache.setApproval(domain, router, approval);
      const res = await mockRedisHelpers.getApproval(domain, router);

      expect(res.value).to.be.eq(approval);
    });
