...
contract RoutersFacetTest is ForgeHelper, Deployer {
  ...
  function test_RouterPermissionsManager__setupRouter_failsIfAlreadyApproved() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(RoutersFacet.RoutersFacet__setupRouter_amountIsZero.selector));
    IConnextFacets(address(connextDiamondProxy)).setupRouter(address(1), address(0), address(0));
  }
  ...
  function test_RouterPermissionsManager__setupRouter_works() public {
    IConnextFacets(address(connextDiamondProxy)).setupRouter(address(1), address(2), address(3));
    assertTrue(IConnextFacets(address(connextDiamondProxy)).getRouterApproval(address(1)));
    assertEq(IConnextFacets(address(connextDiamondProxy)).getRouterOwner(address(1)), address(2));
    assertEq(IConnextFacets(address(connextDiamondProxy)).getRouterRecipient(address(1)), address(3));
  }
  ...
  function test_RouterPermissionsManager__removeRouter_works() public {
    IConnextFacets(address(connextDiamondProxy)).setupRouter(address(1), address(1), address(1));
  ...
  function test_RouterPermissionsManager__setRouterRecipient_worksIfOwnerEmpty() public {
    ...
    vm.prank(_router);
    IConnextFacets(address(connextDiamondProxy)).setRouterRecipient(_router, address(2));
    assertEq(IConnextFacets(address(connextDiamondProxy)).getRouterRecipient(_router), address(2));
  }
  ...
  function test_RouterPermissionsManager__proposeRouterOwner_works() public {
    ...
    vm.prank(address(3));
    IConnextFacets(address(connextDiamondProxy)).proposeRouterOwner(_router, address(2));
    assertEq(IConnextFacets(address(connextDiamondProxy)).getProposedRouterOwner(_router), address(2));
  }
  ...
  function test_RouterPermissionsManager__acceptProposedRouterOwner_failsIfNotProposedWhenProposedSet() public {
    address _router = address(1);
    IConnextFacets(address(connextDiamondProxy)).setupRouter(_router, address(this), address(3));
    IConnextFacets(address(connextDiamondProxy)).proposeRouterOwner(_router, address(1));
  ...
  function test_RouterPermissionsManager__acceptProposedRouterOwner_worksWhenProposedAndOwnerNotSet() public {
    ...
    vm.prank(_router);
    IConnextFacets(address(connextDiamondProxy)).acceptProposedRouterOwner(_router);
    assertEq(IConnextFacets(address(connextDiamondProxy)).getRouterOwner(_router), _router);
  }
  ...
  function test_RouterPermissionsManager__acceptProposedRouterOwner_worksWhenProposedIsSet() public {
    ...
    IConnextFacets(address(connextDiamondProxy)).acceptProposedRouterOwner(_router);
    assertEq(IConnextFacets(address(connextDiamondProxy)).getRouterOwner(_router), address(1));
  }
}
