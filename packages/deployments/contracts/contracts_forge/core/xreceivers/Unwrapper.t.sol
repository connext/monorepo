...
import "../../utils/XReceiverHelper.sol";
import "../../utils/Mock.sol";

contract UnwrapperTest is XReceiverHelper {
  // ============ Events ============
  ...
  function setUp() public {
    ...
    vm.prank(OWNER);
    unwrapper = new Unwrapper(_connext, MOCK_WRAPPER);
    assertEq(unwrapper.CONNEXT(), _connext);
  ...
  function test_Unwrapper__xReceive_works(uint256 amount) public {
    ...
    vm.prank(_connext);
    unwrapper.xReceive(_transferId, amount, asset, _originSender, _origin, callData);
  ...
  function test_Unwrapper__xReceive_worksIfRecipientIsZero() public {
    ...
    vm.prank(_connext);
    unwrapper.xReceive(_transferId, amount, asset, _originSender, _origin, callDataNoRecipient);
  ...
  function test_Unwrapper__xReceive_failsIfNotConnext() public {
    ...
    unwrapper.xReceive(_transferId, amount, asset, _originSender, _origin, callDataWithRecipient);
  }
  ...
  function test_Unwrapper__xReceive_transfersIfNonWrapperAsset() public {
    address asset = _asset; // xReceive will be getting a random non-wrapper erc20 asset.
    uint256 amount = 10 ether;
  ...
  function test_Unwrapper__xReceive_failsIfWrapperFailsToWithdraw() public {
    ...
    unwrapper.xReceive(_transferId, amount, MOCK_WRAPPER, _originSender, _origin, callDataWithRecipient);
  }
