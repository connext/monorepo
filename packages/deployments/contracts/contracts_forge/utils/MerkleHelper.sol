...
/// @dev Logic borrowed from https://github.com/ohhkaneda/Merkle-Proof-Generator-and-Validator-in-Solidity-
abstract contract MerkleHelper {
  function getMerkleProof(bytes32[] memory tree, uint256 node) public pure returns (bytes32[] memory) {
    require(tree.length > 1, "won't generate proof for single leaf");
    // The size of the proof is equal to the ceiling of log2(numLeaves)
    bytes32[] memory result = new bytes32[](log2ceil_naive(tree.length));
    uint256 pos = 0;

    while (tree.length > 1) {
      if (node & 0x1 == 1) {
        result[pos] = tree[node - 1];
      } else if (node + 1 == tree.length) {
  ...
  function log2ceil_naive(uint256 x) private pure returns (uint256) {
    uint256 ceil = 0;
    uint256 pOf2;
  ...
  function log2ceil_bitmagic(uint256 x) private pure returns (uint256) {
    if (x <= 1) {
      return 0;
    }
    uint256 msb;
