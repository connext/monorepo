...
import { ConnextHandlerAbi, TokenRegistryAbi, AssetFacetAbi, BridgeFacetAbi, InboundMessageFacetAbi } from "./abi";
...
const connextAbi = ConnextHandlerAbi.concat(
  AssetFacetAbi.filter((k) => k.includes("event"))
    ...
    .concat(InboundMessageFacetAbi.filter((k) => k.includes("event"))),
);
