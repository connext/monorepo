...
contract RoutersFacet is BaseConnextFacet {
   ...
   * are *not* on the canonical domain, and the canonical asset otherwise.
   * @param _amount - The amount of liquidity to add for the router
   * @param _asset - The address of the bridge representation of the asset
   * @param _router - The router you are adding liquidity on behalf of
   */
  function _addLiquidityForRouter(uint256 _amount, address _asset, address _router) internal {
    ...
    TokenId memory canonical = s.representationToCanonical[_asset];
    if (canonical.domain == 0 && canonical.id == bytes32(0)) {
      ...
      canonical.id = TypeCasts.addressToBytes32(_asset);
    }
    ...
    s.routerBalances[_router][_asset] += _amount;

    emit RouterLiquidityAdded(_router, _asset, key, _amount, msg.sender);
  }
