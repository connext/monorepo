...
contract BaseConnextFacet {
  ...
  error BaseConnextFacet__getAsset_assetNotFound();
  ...
  /**
   * @notice Returns the assets for given canonical information
   */
  function _getAsset(bytes32 _key) internal view returns (address) {
    address asset = AssetLogic.getConfig(_key).adopted;
    if (asset == address(0)) {
      revert BaseConnextFacet__getAsset_assetNotFound();
    }
    return asset;
  }
  ...
  function _getAsset(bytes32 _key, bytes32 _id, uint32 _domain) internal view returns (address) {
    return AssetLogic.getAsset(_key, _id, _domain, s);
  }
