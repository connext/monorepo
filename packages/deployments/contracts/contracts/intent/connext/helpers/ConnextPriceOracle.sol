...
contract ConnextPriceOracle is PriceOracle, ProposedOwnable {
  ...
  function getTokenPrice(address _asset) public view override returns (uint256, uint256) {
    address tokenAddress = _asset;
  ...
  function getPriceFromOracle(address _asset) public view returns (uint256) {
    uint256 chainLinkPrice = getPriceFromChainlink(_asset);
    return chainLinkPrice;
  }

  function getPriceFromChainlink(address _asset) public view returns (uint256) {
