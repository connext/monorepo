 ...
 * `delegatecall` usage as in the original contract. Additionally, the `payable` modifier was
 * removed.
 */
contract MultiSend {
  ...
  function multiSend(bytes memory transactions) public {
    require(address(this) == multisendSingleton, "MultiSend should not be called via delegatecall");
