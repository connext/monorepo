...
library DeployerFunctions{
    ...
    function deploy_Unwrapper(
        Deployer deployer,
        string memory name 
        ,address  connext,address  wrapper
    ...
    function deploy_ProposedOwnableFacet(
        ...
        bytes memory args = abi.encode();
        return ProposedOwnableFacet(DefaultDeployerFunction.deploy(deployer, name, Artifact_ProposedOwnableFacet, args, options));
    }
    ...
    function deploy_RelayerProxyHub(
        Deployer deployer,
        string memory name 
        ,address  _connext,address  _spokeConnector,address  _gelatoRelayer,address  _feeCollector,address  _rootManager,address  _keep3r,address  _autonolas,uint8  _autonolasPriority,uint256  _propagateCooldown,address[] memory _hubConnectors,uint32[] memory _hubConnectorChains
    ) internal returns (RelayerProxyHub) {
        bytes memory args = abi.encode(_connext,_spokeConnector,_gelatoRelayer,_feeCollector,_rootManager,_keep3r,_autonolas,_autonolasPriority,_propagateCooldown,_hubConnectors,_hubConnectorChains);
    ...
    function deploy_ERC20(
        Deployer deployer,
        string memory name 
        ,uint8  decimals_,string memory name_,string memory symbol_,string memory version_
    ...
    function deploy_ConnextPriceOracle(
        ...
        bytes memory args = abi.encode(_wrapped);
        return ConnextPriceOracle(DefaultDeployerFunction.deploy(deployer, name, Artifact_ConnextPriceOracle, args, options));
    }
    
    function deploy_RelayerProxy(
        Deployer deployer,
        string memory name 
        ,address  _connext,address  _spokeConnector,address  _gelatoRelayer,address  _feeCollector,address  _keep3r,address  _autonolas,uint8  _autonolasPriority
    ) internal returns (RelayerProxy) {
        bytes memory args = abi.encode(_connext,_spokeConnector,_gelatoRelayer,_feeCollector,_keep3r,_autonolas,_autonolasPriority);
    ...
    function deploy_ConnextDiamond(
        Deployer deployer,
        string memory name 
        ,address  _contractOwner,_ConnextDiamond.IDiamondCut.FacetCut[] memory _diamondCut,_ConnextDiamond.Initialization[] memory _initializations
    ...
    function deploy_BridgeToken(
        Deployer deployer,
        string memory name 
        ,uint8  decimals_,string memory name_,string memory symbol_
    ...
    function deploy_UpgradeBeaconController(
        ...
        bytes memory args = abi.encode();
        return UpgradeBeaconController(DefaultDeployerFunction.deploy(deployer, name, Artifact_UpgradeBeaconController, args));
    }
    function deploy_UpgradeBeaconController(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode();
        return UpgradeBeaconController(DefaultDeployerFunction.deploy(deployer, name, Artifact_UpgradeBeaconController, args, options));
    }
    
    function deploy_UpgradeBeaconProxy(
        Deployer deployer,
        string memory name 
        ,address  _upgradeBeacon,bytes memory _initializationCalldata
    ...
    function deploy_UpgradeBeaconProxy(
        ...
        bytes memory args = abi.encode(_upgradeBeacon,_initializationCalldata);
        return UpgradeBeaconProxy(DefaultDeployerFunction.deploy(deployer, name, Artifact_UpgradeBeaconProxy, args, options));
    }
    
    function deploy_UpgradeBeacon(
        Deployer deployer,
        string memory name 
        ,address  _initialImplementation,address  _controller
    ...
    function deploy_OptimismSpokeConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _processGas,uint256  _reserveGas,uint256  _delayBlocks,address  _merkle,address  _watcherManager,uint256  _gasCap
    ) internal returns (OptimismSpokeConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_gasCap);
        return OptimismSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_OptimismSpokeConnector, args));
    }
    function deploy_OptimismSpokeConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_gasCap);
        return OptimismSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_OptimismSpokeConnector, args, options));
    }
    
    function deploy_OptimismHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,address  _optimismPortal,uint256  _gasCap
    ) internal returns (OptimismHubConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_optimismPortal,_gasCap);
    ...
    function deploy_OptimismHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_optimismPortal,_gasCap);
        return OptimismHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_OptimismHubConnector, args, options));
    }
    
    function deploy_PolygonZkHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint32  _mirrorNetworkId
    ...
    function deploy_PolygonZkHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_mirrorNetworkId);
        return PolygonZkHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_PolygonZkHubConnector, args, options));
    }
    
    function deploy_PolygonZkSpokeConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _processGas,uint256  _reserveGas,uint256  _delayBlocks,address  _merkle,address  _watcherManager,uint32  _mirrorNetworkId
    ) internal returns (PolygonZkSpokeConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_mirrorNetworkId);
        return PolygonZkSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_PolygonZkSpokeConnector, args));
    }
    function deploy_PolygonZkSpokeConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_mirrorNetworkId);
        return PolygonZkSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_PolygonZkSpokeConnector, args, options));
    }
    
    function deploy_ArbitrumHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,address  _outbox,uint256  _maxSubmissionCostCap,uint256  _maxGasCap,uint256  _gasPriceCap
    ) internal returns (ArbitrumHubConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_outbox,_maxSubmissionCostCap,_maxGasCap,_gasPriceCap);
    ...
    function deploy_ArbitrumHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_outbox,_maxSubmissionCostCap,_maxGasCap,_gasPriceCap);
        return ArbitrumHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ArbitrumHubConnector, args, options));
    }
    
    function deploy_ArbitrumSpokeConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _processGas,uint256  _reserveGas,uint256  _delayBlocks,address  _merkle,address  _watcherManager
    ) internal returns (ArbitrumSpokeConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return ArbitrumSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ArbitrumSpokeConnector, args));
    }
    function deploy_ArbitrumSpokeConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return ArbitrumSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ArbitrumSpokeConnector, args, options));
    }
    
    function deploy_MultichainHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _mirrorChainId,uint256  _gasCap
    ) internal returns (MultichainHubConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_mirrorChainId,_gasCap);
        return MultichainHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_MultichainHubConnector, args));
    }
    function deploy_MultichainHubConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_mirrorChainId,_gasCap);
        return MultichainHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_MultichainHubConnector, args, options));
    }
    
    function deploy_MultichainSpokeConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _processGas,uint256  _reserveGas,uint256  _delayBlocks,address  _merkle,address  _watcherManager,uint256  _mirrorChainId,uint256  _gasCap
    ) internal returns (MultichainSpokeConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_mirrorChainId,_gasCap);
        return MultichainSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_MultichainSpokeConnector, args));
    }
    function deploy_MultichainSpokeConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_mirrorChainId,_gasCap);
        return MultichainSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_MultichainSpokeConnector, args, options));
    }
    
    function deploy_AdminHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _rootManager
    ...
    function deploy_PolygonSpokeConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return PolygonSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_PolygonSpokeConnector, args, options));
    }
    
    function deploy_PolygonHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,address  _checkPointManager
    ...
    function deploy_PolygonHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_checkPointManager);
        return PolygonHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_PolygonHubConnector, args, options));
    }
    ...
    function deploy_ICheckpointManager(
        ...
        bytes memory args = abi.encode(_checkpointManager,_fxRoot);
        return ICheckpointManager(DefaultDeployerFunction.deploy(deployer, name, Artifact_ICheckpointManager, args, options));
    }
    
    function deploy_ConsensysHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector
    ...
    function deploy_ConsensysHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector);
        return ConsensysHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ConsensysHubConnector, args, options));
    }
    ...
    function deploy_ConsensysSpokeConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return ConsensysSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ConsensysSpokeConnector, args));
    }
    function deploy_ConsensysSpokeConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return ConsensysSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ConsensysSpokeConnector, args, options));
    }
    
    function deploy_WormholeHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _gasCap,uint16  _mirrorWormholeChainId
    ) internal returns (WormholeHubConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_gasCap,_mirrorWormholeChainId);
    ...
    function deploy_WormholeHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_gasCap,_mirrorWormholeChainId);
        return WormholeHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_WormholeHubConnector, args, options));
    }
    
    function deploy_WormholeSpokeConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _processGas,uint256  _reserveGas,uint256  _delayBlocks,address  _merkle,address  _watcherManager,uint256  _gasCap,uint16  _mirrorWormholeChainId
    ) internal returns (WormholeSpokeConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_gasCap,_mirrorWormholeChainId);
        return WormholeSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_WormholeSpokeConnector, args));
    }
    function deploy_WormholeSpokeConnector(
        Deployer deployer,
        string memory name,
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_gasCap,_mirrorWormholeChainId);
        return WormholeSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_WormholeSpokeConnector, args, options));
    }
    ...
    function deploy_MainnetSpokeConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return MainnetSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_MainnetSpokeConnector, args, options));
    }
    ...
    function deploy_ZkSyncSpokeConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager);
        return ZkSyncSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ZkSyncSpokeConnector, args, options));
    }
    
    function deploy_ZkSyncHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _gasCap
    ...
    function deploy_ZkSyncHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_gasCap);
        return ZkSyncHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_ZkSyncHubConnector, args, options));
    }
    
    function deploy_GnosisSpokeConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _processGas,uint256  _reserveGas,uint256  _delayBlocks,address  _merkle,address  _watcherManager,uint256  _gasCap,uint256  _mirrorChainId
    ) internal returns (GnosisSpokeConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_gasCap,_mirrorChainId);
    ...
    function deploy_GnosisSpokeConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_processGas,_reserveGas,_delayBlocks,_merkle,_watcherManager,_gasCap,_mirrorChainId);
        return GnosisSpokeConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_GnosisSpokeConnector, args, options));
    }
    
    function deploy_GnosisHubConnector(
        Deployer deployer,
        string memory name 
        ,uint32  _domain,uint32  _mirrorDomain,address  _amb,address  _rootManager,address  _mirrorConnector,uint256  _gasCap,uint256  _mirrorChainId
    ) internal returns (GnosisHubConnector) {
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_gasCap,_mirrorChainId);
    ...
    function deploy_GnosisHubConnector(
        ...
        bytes memory args = abi.encode(_domain,_mirrorDomain,_amb,_rootManager,_mirrorConnector,_gasCap,_mirrorChainId);
        return GnosisHubConnector(DefaultDeployerFunction.deploy(deployer, name, Artifact_GnosisHubConnector, args, options));
    }
    ...
    function deploy_RootManager(
        Deployer deployer,
        string memory name 
        ,uint256  _delayBlocks,address  _merkle,address  _watcherManager
