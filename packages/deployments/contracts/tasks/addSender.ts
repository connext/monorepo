...
export default task("add-sender", "Add sender to connector whitelist")
  ...
  .addParam(
    "chain",
    "The chain the connector should be connecting (i.e. 420 if you are enlisting senders for optimism-goerli)",
  )
  .addOptionalParam("connectorAddress", "Override connector address")
  .addOptionalParam("env", "Environment of contracts")
  .setAction(
    async (
      { sender, connectorAddress: _connectorAddress, env: _env, chain: _chain }: TaskArgs,
      { deployments, ethers },
    ) => {
      ...
      const connectorName = getDeploymentName(
        `${protocol.configs[chain].prefix}${+network.chainId === protocol.hub ? "L1" : "L2"}Connector`,
        env,
      );
