...
describe("ProposedOwnableUpgradeable.sol", () => {
  ...
  describe("proposeRouterOwnershipRenunciation", () => {
    it("should fail if router ownership is already renounced", async () => {
      ...
      await expect(proposedOwnable.connect(wallet).proposeRouterOwnershipRenunciation()).to.be.revertedWith(
        "ProposedOwnableUpgradeable__proposeRouterOwnershipRenunciation_038",
      );
    });
  ...
  describe("renounceRouterOwnership", () => {
    it("should fail if router ownership is already renounced", async () => {
      await renounceRouterOwnership();
      await expect(proposedOwnable.connect(wallet).renounceRouterOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceRouterOwnership_038");
    });

    it("should fail if there is no proposal in place", async () => {
      await expect(proposedOwnable.connect(wallet).renounceRouterOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceRouterOwnership_037");
    });

    it("should fail if delay has not elapsed", async () => {
      await proposeRouterOwnershipRenunciation();
      await expect(proposedOwnable.connect(wallet).renounceRouterOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceRouterOwnership_030");
    });
  ...
  describe("proposeAssetOwnershipRenunciation", () => {
    it("should fail if asset ownership already renounced", async () => {
      await renounceAssetOwnership();
      await expect(proposedOwnable.connect(wallet).proposeAssetOwnershipRenunciation()).to.be.revertedWith("ProposedOwnableUpgradeable__proposeAssetOwnershipRenunciation_038");
    });
  ...
  describe("renounceAssetOwnership", () => {
    it("should fail if asset ownership is already renounced", async () => {
      await renounceAssetOwnership();
      await expect(proposedOwnable.connect(wallet).renounceAssetOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceAssetOwnership_038");
    });

    it("should fail if no proposal was made", async () => {
      await expect(proposedOwnable.connect(wallet).renounceAssetOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceAssetOwnership_037");
    });

    it("should fail if delay has not elapsed", async () => {
      await proposeAssetOwnershipRenunciation();
      await expect(proposedOwnable.connect(wallet).renounceAssetOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceAssetOwnership_030");
    });
  ...
  describe("proposeNewOwner", () => {
    it("should fail if not called by owner", async () => {
      await expect(proposedOwnable.connect(other).proposeNewOwner(constants.AddressZero)).to.be.revertedWith("ProposedOwnableUpgradeable__onlyOwner_029");
    });

    it("should fail if proposing the same address as what is already proposed", async () => {
      await proposeNewOwner(other.address);
      await expect(proposedOwnable.connect(wallet).proposeNewOwner(other.address)).to.be.revertedWith("ProposedOwnableUpgradeable__proposeNewOwner_036");
    });

    it("should fail if proposing the owner", async () => {
      await expect(proposedOwnable.connect(wallet).proposeNewOwner(wallet.address)).to.be.revertedWith("ProposedOwnableUpgradeable__proposeNewOwner_038");
    });
  ...
  describe("renounceOwnership", () => {
    it("should fail if there was no proposal", async () => {
      await expect(proposedOwnable.connect(wallet).renounceOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceOwnership_037");
    });

    it("should fail if the delay hasnt elapsed", async () => {
      await proposeNewOwner(constants.AddressZero);
      await expect(proposedOwnable.connect(wallet).renounceOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceOwnership_030");
    });
    ...
    it("should fail if the proposed != address(0)", async () => {
      ...
      await expect(proposedOwnable.connect(wallet).renounceOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__renounceOwnership_036");
    });

    it("should fail if not called by owner", async () => {
      await proposeNewOwner(constants.AddressZero);
      await expect(proposedOwnable.connect(other).renounceOwnership()).to.be.revertedWith("ProposedOwnableUpgradeable__onlyOwner_029");
    });
  ...
  describe("acceptProposedOwner", () => {
    it("should fail if not called by proposed", async () => {
      await proposeNewOwner(other.address);
      await expect(proposedOwnable.connect(wallet).acceptProposedOwner()).to.be.revertedWith("ProposedOwnableUpgradeable__onlyProposed_035");
    });
    ...
    it("should fail if there is no effective change in ownership", async () => {
      ...
      // Then try again
      await expect(proposedOwnable.connect(other).acceptProposedOwner()).to.be.revertedWith("ProposedOwnableUpgradeable__acceptProposedOwner_038");
    });

    it("should fail if delay has not elapsed", async () => {
      await proposeNewOwner(other.address);
      await expect(proposedOwnable.connect(other).acceptProposedOwner()).to.be.revertedWith("ProposedOwnableUpgradeable__acceptProposedOwner_030");
    });
