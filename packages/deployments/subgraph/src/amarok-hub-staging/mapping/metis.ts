...
import { NewConnector, MessageProcessed } from "../../../generated/MetisHubConnector/MetisHubConnector";
import { MetisConnectorMeta, RootMessageProcessed } from "../../../generated/schema";

const DEFAULT_METIS_HUB_CONNECTOR_META_ID = "METIS_HUB_CONNECTOR_META_ID";

/// MARK - BNB
export function handleMetisNewConnector(event: NewConnector): void {
  let meta = MetisConnectorMeta.load(DEFAULT_METIS_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new MetisConnectorMeta(DEFAULT_METIS_HUB_CONNECTOR_META_ID);
  }
...
export function handleMetisMessageProcessed(event: MessageProcessed): void {
  let meta = MetisConnectorMeta.load(DEFAULT_METIS_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new MetisConnectorMeta(DEFAULT_METIS_HUB_CONNECTOR_META_ID);
  }
