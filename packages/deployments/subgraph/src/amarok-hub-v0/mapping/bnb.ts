...
import { NewConnector, MessageProcessed } from "../../../generated/BnbHubConnector/BnbHubConnector";
import { BnbConnectorMeta, RootMessageProcessed } from "../../../generated/schema";

const DEFAULT_BNB_HUB_CONNECTOR_META_ID = "BNB_HUB_CONNECTOR_META_ID";

/// MARK - BNB
export function handleBnbNewConnector(event: NewConnector): void {
  let meta = BnbConnectorMeta.load(DEFAULT_BNB_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new BnbConnectorMeta(DEFAULT_BNB_HUB_CONNECTOR_META_ID);
  }
...
export function handleBnbMessageProcessed(event: MessageProcessed): void {
  let meta = BnbConnectorMeta.load(DEFAULT_BNB_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new BnbConnectorMeta(DEFAULT_BNB_HUB_CONNECTOR_META_ID);
  }
