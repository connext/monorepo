...
import { NewConnector, MessageProcessed } from "../../../generated/ArbitrumHubConnector/ArbitrumHubConnector";
import { ArbitrumConnectorMeta, RootMessageProcessed } from "../../../generated/schema";

const DEFAULT_ARBITRUM_HUB_CONNECTOR_META_ID = "ARBITRUM_HUB_CONNECTOR_META_ID";

/// MARK - ARBITRUM
export function handleArbitrumNewConnector(event: NewConnector): void {
  let meta = ArbitrumConnectorMeta.load(DEFAULT_ARBITRUM_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new ArbitrumConnectorMeta(DEFAULT_ARBITRUM_HUB_CONNECTOR_META_ID);
  }
...
export function handleArbitrumMessageProcessed(event: MessageProcessed): void {
  let meta = ArbitrumConnectorMeta.load(DEFAULT_ARBITRUM_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new ArbitrumConnectorMeta(DEFAULT_ARBITRUM_HUB_CONNECTOR_META_ID);
  }
