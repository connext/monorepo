...
import { NewConnector, MessageProcessed } from "../../../generated/ZkSyncHubConnector/ZkSyncHubConnector";
import { ZkSyncConnectorMeta, RootMessageProcessed } from "../../../generated/schema";

const DEFAULT_ZKSYNC_HUB_CONNECTOR_META_ID = "ZKSYNC_HUB_CONNECTOR_META_ID";

/// MARK - ZKSYNC
export function handleZkSyncNewConnector(event: NewConnector): void {
  let meta = ZkSyncConnectorMeta.load(DEFAULT_ZKSYNC_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new ZkSyncConnectorMeta(DEFAULT_ZKSYNC_HUB_CONNECTOR_META_ID);
  }
...
export function handleZkSyncMessageProcessed(event: MessageProcessed): void {
  let meta = ZkSyncConnectorMeta.load(DEFAULT_ZKSYNC_HUB_CONNECTOR_META_ID);
  if (meta == null) {
    meta = new ZkSyncConnectorMeta(DEFAULT_ZKSYNC_HUB_CONNECTOR_META_ID);
  }
