...
describe("Helpers:Address", () => {
  describe("#getAddressFromPublicKey", () => {
    it(`Should return address ${testAddress} from pub key`, () => {
      expect(getAddressFromPublicKey(testPublicKey)).to.be.deep.eq(testAddress);
    });
    ...
    it(`Should throw if pubKey is invalid`, () => {
      ...
      expect(() => getAddressFromPublicKey(testPublicKey2)).throw();
    });
  });
  ...
  describe("#compressPublicKey", () => {
    it(`Should get compressed version of public key`, () => {
      ...
      expect(compressPublicKey(testPublicKey)).to.be.deep.equal(compressedForm);
    });
  });
