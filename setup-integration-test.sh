...
MNEMONIC=${DEFAULT_MNEMONIC} ENV=${NXTP_ENVIRONMENT} yarn workspace @connext/nxtp-contracts hardhat deploy --network localhost_1337 --reset --tags local
echo "Deployed contracts to 1337"

echo "Deploying contracts to 1338..."
MNEMONIC=${DEFAULT_MNEMONIC} ENV=${NXTP_ENVIRONMENT} yarn workspace @connext/nxtp-contracts hardhat deploy --network localhost_1338 --reset --tags local
