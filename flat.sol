...
contract FulfillInterpreter is ReentrancyGuard, IFulfillInterpreter {
   ...
   */
  function getTransactionManager() external view override returns (address) {
    ...
    )
  {
    ...
    // Emit event
    emit Executed(transactionId, callTo, assetId, fallbackAddress, amount, callData, returnData, success, isContract);
...
contract TransactionManager is ReentrancyGuard, ProposedOwnable, ITransactionManager {
  ...
  function prepare(PrepareArgs calldata args) external payable override nonReentrant returns (TransactionData memory) {
    ...
    if (args.invariantData.sendingChainId == _chainId) {
      ...
      // contract
      amount = transferAssetToContract(args.invariantData.sendingAssetId, args.amount);

      // Store the transaction variants. This happens after transferring to
      // account for fee on transfer tokens
      variantTransactionData[digest] = hashVariantTransactionData(amount, args.expiry, block.number);
    } else {
    ...
    emit TransactionPrepared(txData.user, txData.router, txData.transactionId, txData, msg.sender, args);
  ...
  function fulfill(FulfillArgs calldata args) external override nonReentrant returns (TransactionData memory) {
...

    {
      ...
      require(
        variantTransactionData[digest] ==
          hashVariantTransactionData(args.txData.amount, args.txData.expiry, args.txData.preparedBlockNumber),
        "#F:019"
      );
    ...
    if (args.txData.sendingChainId == _chainId) {
      ...
      require(
        recoverFulfillSignature(args.txData.transactionId, args.relayerFee, _chainId, address(this), args.signature) ==
          args.txData.user,
        "#F:022"
      );
      ...
      (success, isContract, returnData) = _receivingChainFulfill(args.txData, args.relayerFee, args.callData);
    }
  ...
  function cancel(CancelArgs calldata args) external override nonReentrant returns (TransactionData memory) {
    ...
    if (args.txData.sendingChainId == _chainId) {
      ...
      LibAsset.transferAsset(args.txData.sendingAssetId, payable(args.txData.sendingChainFallback), args.txData.amount);
    } else {
      ...
      if (args.txData.expiry >= block.timestamp) {
        ...
        // Validate signature
        require(
          msg.sender == args.txData.user ||
            recoverCancelSignature(args.txData.transactionId, _chainId, address(this), args.signature) ==
            args.txData.user,
          "#C:022"
        );
    ...
    // Emit event
    emit TransactionCancelled(args.txData.user, args.txData.router, args.txData.transactionId, args, msg.sender);
    ...
    )
  {
    ...
    if (txData.callTo == address(0)) {
      ...
      return
        interpreter.execute{value: isNativeAsset ? toSend : 0}(
