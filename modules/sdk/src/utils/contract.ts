...
export class TransactionManagerListener {
  ...
  private constructor(public readonly provider: providers.Web3Provider, public readonly chainId: number) {}

  static async connect(provider: providers.Web3Provider): Promise<TransactionManagerListener> {
    const { chainId } = await provider.getNetwork();

    const watcher = new TransactionManagerListener(provider, chainId);
    watcher.establishListeners();
    return watcher;
  }

  private establishListeners() {
    const { address, abi } = getTransactionManagerContract(this.chainId);
